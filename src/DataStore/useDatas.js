...
function useDatas() {
  ...
  const doSignIn = (auth) => {
    ...
    axios.defaults.headers.common.Authorization = import.meta.env.VITE_APP_AUTH_HEADER_TYPE + ` ${auth.jwt}`;
  ...
  const doSignOut = () => {
    ...
    // setting default auth for every request
    axios.defaults.headers.common.Authorization = import.meta.env.VITE_APP_AUTH_HEADER_TYPE + " default_header";
